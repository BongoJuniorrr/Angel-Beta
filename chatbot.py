...
model = tf.keras.models.load_model('./chatbot_model/chatbotmodel.h5', compile=False)
model.compile()
...
def reply(message):
    ...
    if res=="object_detection":
        if "what is it" not in message and "what is this" not in message and "caption" not in message:
            res="answer"
    if res=="text_scanning":
        if "read this" not in message and "scan this" not in message:
            res="answer"
    if "playstation" in res:
        res=""
    return res
